...
```{r}
ggplot(temp2 %>% group_by(Focal) %>% summarise(Sd = mean(statistic)), aes(x = Focal + 1, y = Sd)) +
  geom_rect(xmin = 22, xmax = 38, ymin = -Inf, ymax = Inf, alpha = .1, color = NA, fill = "grey90") + 
  geom_line() + theme_bw() +
  # geom_vline(xintercept = 30) +
...
<!-- wl.df2 <- wl.df %>% mutate(day = day + Window.length) -->
<!-- wl.df3 <- rbind(wl.df, wl.df2) -->

<!-- ggplot(wl.df3 %>% group_by(day) %>% summarise(iota = sd(iota)), aes(x = day, y = iota)) + geom_line() -->
